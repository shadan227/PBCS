...
while True:
    ...
    if choice == 1:
        ...
        for doc in cursor:
            ...
            Score = int(input())
            db[Collection].update_one({"_id":doc["_id"]}, {"$set": {"Score": Score}}, upsert=False)
        cursor.close()
    elif choice == 2:
        cursor = db[Collection].find()
        print("Score List")
        ...
        Score = int(input())
        db[Collection].update_one({"_id":_id}, {"$set": {"Score": Score}}, upsert=False)
        cursor.close()
    elif choice==3:
        cursor = db[Collection].find()
        print("Enter Scores for the following")
        ...
        print("Enter Scores for the following")
        for doc in cursor:
            db[Collection].update_one({"_id":doc["_id"]}, {"$set": {"Score": 0}}, upsert=False)
        cursor.close()
    elif choice==5:
        break
    print()
